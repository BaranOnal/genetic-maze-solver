...
def draw_panel(screen, font, small_font, tiny_font, pop, best_agent, alive_count, step_counter, all_time_best_score, speed_multiplier):
    ...
    # Title
    title = font.render("NEUROEVOLUTION", True, YELLOW)
    screen.blit(title, (px + 20, y))
    y += 36

    sub = tiny_font.render("Neuroevolution Maze Solver", True, DIM_TEXT)
    ...
    sim_stats = [
        ...
        ("DNA Length",  str(best_agent.dna_length)),
        ("Alive",       str(alive_count)),
        ("Step",        f"{step_counter} / {pop.max_steps}"),
    ]
...
def main():
    ...
    pygame.display.set_caption("Neuroevolution Maze Solver")
    ...
    pop = Population(environment, pop_size=500, max_steps=100)
    ...
    while running:
        ...
        while step_counter < pop.max_steps and running:
